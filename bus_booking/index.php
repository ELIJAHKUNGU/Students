<?php
include 'header.php';

?>
        <!-- !Primary Navigation -->
        <div class="banner">
            <div class="col-sm-6">
                <div class="container">
                    <div class="p-5">
                        <h1>Easy Way To Make Your Small Moves</h1>
                        <p>Rent our Powerful and automatic transmission delivery vans on lower rates now!</p>
                        <div class="pl-5 mt-5">
                            <div class="d-flex">
                                <button class="btn pl-5 pr-5 pt-3 pb-3 btn-primary">Learn More</button>


                                <a href="./dashboard.php">
                                <button class="btn btn-warning pl-5 pr-5 ml-3">Book Now</button>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </header>
    <section>
        <div class="container browser shadow pt-5 pb-5">
            <div class="row">
                <div class="col-sm-3">
                    <label for="">Choose a Pickup location</label>
                    <select name="" class="form-control" id="">
                      <option value="">Choose a Pickup location</option>
                      <option value="">Juja</option>
                      <option value="">Kiambu</option>
                      <option value="">Kilimani</option>
                      <option value="">Thika</option>
                    </select>
                </div>
                <div class="col-sm-3">
                    <div class="d-block">
                        <label for="">Select Date</label>
                        <input type="date" class="form-control" name="" id="">
                    </div>
                </div>
                <div class="col-sm-3">
                    <label for="">Select Model</label>
                    <select name="" id="" class="form-control">
                    <option value="">Select the Model</option>
                    <option value="">Easy Coach</option>
                    <option value="">Modern Coast</option>
                    <option value="">Express </option>
                    <option value="">Flight one</option>
                  </select>
                </div>
                <div class="col-sm-3">
                    <a href="./dashboard.php"><button class="btn btn-warning  pl-5 pr-5">Browse Vehicles </button></a>
                    </select>
                </div>
            </div>
        </div>
    </section>
    <!-- <section>
        <div class="servicess">
            <div class="container">
                <div class="pt-5">
                    <div class="pt-5">
                        <div class="d-flex justify-content-center">
                            <img src="./images/logo.png" alt="" srcset="">

                        </div>
                        <div class="text-center">
                            <p class="mt-2">Large Growing Fleet of Bus and Vans</p>
                            <h1 style="font-weight: 900; font-size: 3rem;">Company Services</h1>
                        </div>
                    </div>
                </div>
                <div class="row mt-4">
                    <div class="col-sm-6  ">
                        <div class="d-flex shadow">
                            <div class="side-image">
                                <img src="./images/p1.jpg" style="min-height: 180px; min-width: 180px; margin-left: 0px;" class="mr-2 img-fluid" alt="" srcset="">
                            </div>
                            <div class="text-side pt-5 ml-4">
                                <h3>Parcel Delivery</h3>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Facere adipisci nostrum numquam possimus repellendus architecto

                                    <div class="mt-4">
                                        <button>Details</button>
                                    </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-6  ">
                        <div class="d-flex shadow">
                            <div class="side-image">
                                <img src="./images/b5.jpg" style="max-height: 400px; width:1050px; object-fit:cover; margin-left: 0px;" class="mr-2 img-fluid" alt="" srcset="">
                            </div>
                            <div class="text-side pt-5 ml-4">
                                <h3>Tours & Safaris</h3>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Facere adipisci nostrum numquam possimus repellendus architecto

                                    <div class="mt-4 mb-4">
                                        <button>Details</button>
                                    </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="row mt-4">
                    <div class="col-sm-6  ">
                        <div class="d-flex shadow">
                            <div class="side-image">
                                <img src="./images/p3.jpg" style="max-height: 400px; max-width: 140px; margin-left: 0px;" class="mr-2 img-fluid" alt="" srcset="">
                            </div>
                            <div class="text-side pt-5 ml-4">
                                <h3>Private Deliveries</h3>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Facere adipisci nostrum numquam possimus repellendus architecto

                                    <div class="mt-4">
                                        <button>Details</button>
                                    </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-6  ">
                        <div class="d-flex shadow">
                            <div class="side-image">
                                <img src="./images/bus.png" style="max-height: 400px; max-width: 140px; margin-left: 0px;" class="mr-2 img-fluid" alt="" srcset="">
                            </div>
                            <div class="text-side pt-5 ml-4">
                                <h3>Long Distance Travel</h3>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Facere adipisci nostrum numquam possimus repellendus architecto

                                    <div class="mt-4">
                                        <button>Details</button>
                                    </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section> -->
    <section>
        <div class="container">
                <div class="pt-5">
                    <div class="pt-5">
                        <div class="d-flex justify-content-center">
                            <img src="./images/logo.png" alt="" srcset="">

                        </div>
                        <div class="text-center">
                            <p class="mt-2">Large Growing Fleet of Bus and Vans</p>
                            <h1 style="font-weight: 900; font-size: 3rem;">Travel in Style</h1>
                        </div>
                    </div>
                </div>
            <img src="./images/bus.png" class="img-fluid" alt="">
        </div>
    </section>


    <div class="text-center text-white ">
        <h6>&copysr; Powered by Autoride LTD &copy; 2021</h6>
    </div>
    </div>
    </div>
    </section>
    <script src="./js/jquery.min.js "></script>
    <script src="./js/bootstrap.min.js "></script>
    <script src="./owl-carousel/js/owl.carousel.min.js "></script>
    <script src="./js/bootstrap.js "></script>
    <script src="./js/index.js "></script>


</body>

</html>